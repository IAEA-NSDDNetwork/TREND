...
C/      Go to 7
C...DVF
      Call Getarg(1,filnam,ilent)
      If(ilent.lt.1)  goto 6
!     Noninteractive input for VAX
      innam = filnam
      newfil=.true.
	Call Getarg(2,filnam,ilent)
	If(ilent.lt.1) then
	   outnam='tt.dat'
	   nclmn=80
	   linpag=23
         Open  (unit=20, file=innam, status='OLD',READONLY)
	   Go to 7
	Else
	   outnam = filnam
         If(outnam.eq.'TT:'.or.outnam.eq.'tt:') then
            outnam='TT.DAT'
         End if
	Endif
	Call Getarg(3,col,ilent)
      If(ilent.lt.1) then
         nclmn=80
         linpag=23
      Else
         Read(unit=col,fmt='(BN,I)') nclmn
         If(nclmn.ne.80.and.nclmn.ne.132) nclmn=80
	   Call Getarg(4,col,ilent)
         If(ilent.lt.1) then
            linpag=23
         Else
            Read(unit=col,fmt='(BN,I)') linpag
         End If
      End If
      Open  (unit=20, file=innam, status='OLD',READONLY)
      Go to 7
...
C...VAX, DVF
      Open  (unit=20, file=innam, status='OLD',READONLY)
C...ANS, UNX
C/      Open  (unit=20, file=innam, status='OLD')
C...VAX, DVF, ANS, UNX
